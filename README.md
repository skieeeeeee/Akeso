...
Two hosts, because the halves have different needs: the frontend is a static
bundle, the API needs a database, a persistent disk and 96 MB of OCR runtime.

- **Frontend → Vercel.** `frontend/vercel.json` is committed.
- **API → Render.** `render.yaml` is committed; `backend/Dockerfile` builds it.

Vercel cannot host this API: uploads are written to disk and would be lost on
every cold start, and `OCR_TIMEOUT_SECONDS=60` exceeds the serverless limit.

### 1. API on Render

1. Push to GitHub, then **New → Blueprint** on Render and pick the repo.
   `render.yaml` defines the web service and a free PostgreSQL instance.
2. Apply. First boot runs migrations and seeds the four demo patients
   (`SEED_DEMO_PATIENTS=true`, idempotent — safe on every restart).
3. Wait for `GET /health` to return `{"status":"ok"}`.
4. Note the URL, e.g. `https://medikiosk-api.onrender.com`.

`DATABASE_URL` needs no editing: Render injects `postgres://…` and the app
rewrites it to the psycopg driver form on load.

### 2. Frontend on Vercel

1. **Add New → Project**, import the repo, and set **Root Directory** to
   `frontend`. Vercel detects Vite; `vercel.json` supplies the SPA rewrite.
...
3. Deploy, and note the domain, e.g. `https://medikiosk.vercel.app`.

### 3. Let the browser talk to the API

The two are different origins, so the API must allow the Vercel one. On
Render, set:

```
CORS_ORIGINS=["https://medikiosk.vercel.app"]
```

A JSON array, no trailing slash, and it must include every domain you use —
Vercel preview deployments each get their own. Then redeploy the API.

### 4. Check it

```bash
curl https://YOUR-API.onrender.com/health
curl https://YOUR-API.onrender.com/api/v1/auth/demo-patients   # expect four
```
...
The deployed defaults are a **prototype**, and the API says so in its logs on
every boot. Two of them mean anyone who knows a mobile number can read that
patient's record:

| Setting | Deployed default | For real patients |
| --- | --- | --- |
| `DEV_FIXED_OTP` | `12345` for every number | `""` |
| `EXPOSE_MOCK_OTP` | `true` — code returned in the response | `false` |
| `JWT_SECRET` | generated by Render | keep it secret |
| `DEBUG` | `false` | `false` |

With both sign-in settings on, **treat the deployment as a demo over
fictional data and do not enter real health information.** There is no SMS
...
- **Uploads do not persist.** Render free instances have no disk, so
  documents are lost on restart. Attach a disk on a paid instance and point
